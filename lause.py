mikä olla-verbi on
mitä muut verbit puhut

mikä päivä tänään on
mitä kieltä sinä puhut

vahva-k p t  heikko-  v d
ei sk st tk
ei 2 vokaalia lopussa vapaa-vapaassa

turku turussa
kylpy kyvyssä
katu kadulla

helsinki helsingissä
irlanti irlannin
kampa kammalla

ystäväni
kirjasi
hänen autonsa
opettajamme
kotimaanne
heidän kurssinsa

nt-nn antaa-annan kääntää-käänan lentää-lennan
tt-t kirjoittaa-kirjoitan ottaa-otan
kk-k nukkua-nukun-nukkuu
k- - lukea-luen pukea-puen
nk-ng onkia-ongin
t-d tietää-tiedän
rt-rr ymmärtää-ymmärran

# partitiivi
partitiivissa ei kpt vaihtelua
vokaali a-ä
taloa valoisaa kylmää
ia-ea-eä a-ä
italiaa vaikeaa pimeää
2 vokaalia-konsonantti ta-tä
10 puutä
maata-puhelinta-kaunista-telivisiota
e tta-ttä
kirjettä-venettä-osoitetta-huonetta

# numero partitiivi
9 pitkää miestä
3 hyvää tietokonetta
monta-vähän-nolla-pari-puoli astetta

# verbi partiviivi
puhua saksaa -rakastaa -etsiä-etsii työpaikkaa
-soittaa -katsoa -ymmärtää -auttaa mua-häntä-meitä-teitä-heitä-niitä

pelaa jalkapalloa
opiskelen historiaa
juhlimme Tiinan syntymäpäivää lauantaina
voinko häiritä sua


mulla on vanha auto
isolla koiralla on luu(bone)
mulla on jano
hänellä on punainen tukka

mulla ei ole autoa
heillä ei ole isoa perhettä
Mialla ja Pialla ei ole testiä huomenna
mulla ei ole jano(ei partitiivia)

# genetiivi -n kanssa
pedro asuu hannan kanssa
olga on pedron ja alexin kanssa kurssilla
hän puhuu opettajan kanssa

# kello
kello on kahdeksan illalla
yhdeltä kahdeksalta yhdeltätoista

kysyn myyjältä
tarkistan varastosta

voinko sovittaa tätä - tämä on sopiva

?????????????????
(sivu 96)
# nukkua kpt
nukun
# hymyillä
hymyilette
# ehtiä
ehtii(have time to)
# laina
lainaan(borrow)
# katsoa
katson
# lukea
luen

# juoda
juon
# nähdä
näen näette näkevät
# myydä
myy
#siivota
siivoaa
# saada
saan

# purra
puren(bite)
# nauraa
nauran mulle
# nousta
nousen
# mennä
menen
# kävellä
kävelen
# käydä
käynk


#haluta
haluan
# pestä
pesen
# osata
osaan


# häiritä
häiritsen  häiritsee
harkitsen(considering)
# valita
valitsen(choose) valinta(choice)
# välittää
välitän(care)
# valittaa
valitan(complain)
# mainita
mainitsen
#tulkita
tulkitsevat(interpret-translate-decode)


kantaa(carry)
kääntää(turn flip over translate)
kaataa(pour)

viedä(take export)


hän ei syö spagettia
me emme opiskele
he eivät tarvitse rahaa

# tavallisesti
posti tulee tavallisesti

korkea-matala
leveä(wide)-kapea(narrow)
viihtyisä(homey)-sotkuinen(messy)

# mistä
kotoa ulkoa sisältä
# missä
kotona ulkona sisällä
# mihin
kotiin ulos sisään

(sivu 135)






# end
